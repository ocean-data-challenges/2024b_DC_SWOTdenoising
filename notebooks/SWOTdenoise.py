...
def variational_regularization_filter_fista(ssh, param, itermax, epsilon, pc_method='gaussian', pc_param=10.):
    ...
    if np.sum(param):#any(param) is not 0.: 
...
def cost_function(mask,hobs, h, param):
    ...
    c_func = 0.5 * ( np.ma.sum(mask*(h - hobs)**2) + (np.ma.sum(param[0]*grad_h)) + (np.ma.sum(param[1]*lap_h**2)) + (np.ma.sum(param[2]*gradlap_h)) )
...
def SWOTdenoise(*args, **kwargs):
    ...
    **kwargs include:
    ...
    print('shape ssh_f',np.shape(ssh_f))
